(function () {
    ...
    startGame.addEventListener('click', function () {
        ...
        gameControl.innerHTML += `<button id="quite">Do you want to quite</button>`
    ...
    function throwDice() {
        ...
        if (gameData.rollSum === 2) {
            ...
            gameData.index ? (gameData.index = 0) : (gameData.index = 1);
            game.innerHTML += `<p>You rolled a 1, switching to other player ${gameData.players[gameData.index]}</p>`;
            setTimeout(setUpTurn, 2000);

        } else {
            gameData.score[gameData.index] = gameData.score[gameData.index] + gameData.rollSum;
            actionArea.innerHTML = `<button id="rollAgain">Roll agin</button> Or <button id="pass">Pass</button>`;
    ...
    function checkWiningCondition() {
        if (gameData.score[gameData.index] > gameData.gameEnd) {
            ...
            actionArea.innerHTML = '';
            document.getElementById('quite').innerHTML = 'Start A New Game';
    ...
    function showCurrentScore() {
        score.innerHTML = `<p>Score for ${gameData.players[0]} is <strong>${gameData.score[0]}</strong> and the score for player ${gameData.players[1]} is <strong>${gameData.score[1]}</strong></p>`
    };
})();